...
def main():
    ...
    #! MODIFIED: Now loading both API keys from the .env file!
    google_api_key = os.getenv("GOOGLE_API_KEY")
    anthropic_api_key = os.getenv("ANTHROPIC_API_KEY")

    #! MODIFIED: Checking for both keys now!
    if not google_api_key or not anthropic_api_key:
        print("ERROR: One or more API keys (GOOGLE_API_KEY, ANTHROPIC_API_KEY) not found in .env file!")
        return

    print("--- activating narrative engine v5.0 ---")
    try:
        #! MODIFIED: Configure the conductor's own brain with the Google Key.
        ...
        for i, micro_prompt in enumerate(five_part_prompts, 1):
            ...
            #! MODIFIED: Passing the ANTHROPIC key to our new Author bot!
            author_command = ['python', os.path.join(PROJECT_ROOT, 'scripts', 'author.py'), '--prompt-file', prompt_part_file, '--output-file', chapter_part_file, '--api-key', anthropic_api_key]
        ...
        for i, initial_part_path in enumerate(initial_generated_paths):
            ...
            for attempt in range(1, args.max_iterations + 1):
                ...
                #! MODIFIED: Passing the GOOGLE key to the Critic bot!
                critic_command = ['python', os.path.join(PROJECT_ROOT, 'scripts', 'critic.py'), '--author-prompt-file', current_prompt_path, '--generated-file', current_part_path, '--output-file', critique_path, '--api-key', google_api_key]
                ...
                #! MODIFIED: Passing the GOOGLE key to the Re-writer bot!
                new_prompt_text = rewrite_prompt_based_on_critique(original_prompt_text, critique_text, google_api_key)
                ...
                #! MODIFIED: Passing the ANTHROPIC key to the Author bot again in the loop!
                author_command = ['python', os.path.join(PROJECT_ROOT, 'scripts', 'author.py'), '--prompt-file', current_prompt_path, '--output-file', current_part_path, '--api-key', anthropic_api_key]
                run_script(author_command)
